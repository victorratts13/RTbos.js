...
setTimeout(() =>{    
        setInterval(function(){ 
            ...
            poloniex.returnChartData('BTC_GNT', '300', start, end, (err, data) => {
                var dataCh = data
                if(dataCh){
                ...
                poloniex.returnBalances((err, balance) => {
                        if(err){
                            ...
                            price = dataCh[0].high;
                            console.log('salso em GNT -> '+balance.GNT)
                            console.log('Saldo em BTC -> '+balance.BTC)
                            console.log(resultRSI[7])
                            console.log(price)
                            var buy = balance.BTC / price;
                            var sell = price * balance.GNT / price;
                            console.log('total sell -> '+ parseFloat(sell.toFixed(8))+ '\n #-----------------------------------')
                            console.log('total buy --> '+ parseFloat(buy.toFixed(8)) + '\n #-----------------------------------' )
                        
                    if(resultRSI[7] > 70){
                        poloniex.sell('BTC_GNT', price, sell , 1, 1, 0, (err, sell) => {console.log(JSON.stringify(sell)+ '\n aguardando Proxima ordem')})
                    }else{
                        console.log(`Aguardando Momento para Venda`)  
                    }if(resultRSI[7] < 30){
                        poloniex.buy('BTC_GNT', price, buy, 0, 1, 0, (err, buy) => {console.log(buy)})
                    }else{
